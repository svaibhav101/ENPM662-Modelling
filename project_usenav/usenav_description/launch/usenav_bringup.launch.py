...
def generate_launch_description():
    ...
    world_path = os.path.join(
        get_package_share_path("usenav_description"), "world", "final_wh2.world.sdf"
    ) 
    ...
    model_arg = DeclareLaunchArgument(
        name="model",
        default_value=urdf_path, description="Absolute path to robot URDF file.",
    )
    ...
    gazebo = IncludeLaunchDescription(
        ...
        launch_arguments=[("gz_args", [" -v 4", " -r ",world_path])],
    )
    ...
    if os.path.isfile(display_rviz_path):
        ...
        print(
            f"==> [usenav_rviz.launch.py] [WARN] RViz config not found: {display_rviz_path}"
        )
